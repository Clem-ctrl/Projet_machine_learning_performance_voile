...
if __name__ == "__main__":
    ...
    with requests.Session() as session:
        ...
        # Ne traiter que la première URL trouvée
        if urls_to_process:
            urls_to_process = urls_to_process[:1]
            source_url = urls_to_process[0]
            print(f"\n--- Début du traitement de la première URL : {source_url} ---")

            try:
                # Utilise la même logique que le scraper pour obtenir le HTML
                scraper = MetasailScraper(source_url, "", "", "", "", session)
                page_source = scraper._scrape_page_with_cli(source_url)

                if page_source:
                    print("\nScraping de la première URL terminé. Le HTML a été affiché.")
                else:
                    print("\nErreur : Le HTML n'a pas pu être récupéré pour la première URL.")

            except Exception as e:
                print(f"Une erreur inattendue est survenue pour l'URL {source_url}: {e}")

        else:
            print("Aucune URL de course n'a été trouvée à partir des URLs de départ.")
